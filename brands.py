...
async def generate_brand_names_stream(
    ...
    of Stages 1 to 5 for AI Name Generation. Rate limited (H-03)."""
    if not request.suggestion_form and not (request.molecule or "").strip():
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Provide either a suggestion_form case or a molecule to generate names for",
...
async def parse_compare_names(
    ...
    """Compare Names' Upload Excel path. Rate limited (L-10) and size capped (H-09)."""
    filename = file.filename or "upload"
    ext = filename.rsplit(".", 1)[-1].lower() if "." in filename else ""
    file_bytes = await file.read()
    if not file_bytes:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, "Uploaded file is empty")
    # M-15: Max file size 50MB (50 * 1024 * 1024)
    MAX_UPLOAD_BYTES = 50 * 1024 * 1024
    if len(file_bytes) > MAX_UPLOAD_BYTES:
        raise HTTPException(status.HTTP_413_REQUEST_ENTITY_TOO_LARGE, "File exceeds maximum allowed size (50MB)")
...
def get_brand_search(
    ...
    # M-05: IDOR verification
    if search.user_id and search.user_id != current_user.id and not current_user.is_superuser:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Access denied to this brand search")
    return search
